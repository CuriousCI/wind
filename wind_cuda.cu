...
int main(int argc, char *argv[]) {
    ...
    particle_locations
        = (int *)malloc(sizeof(int) * (size_t)rows * (size_t)columns);

    if (flow == NULL || flow_copy == NULL || particle_locations == NULL) {
        fprintf(
            stderr,
            "-- Error allocating culture structures for size: %d x %d \n",
            rows,
            columns
        );
        exit(EXIT_FAILURE);
    ...
    for (iter = 1; iter <= max_iter && max_var > var_threshold; iter++) {
        ...
        if (iter % STEPS == 1) {
            for (j = inlet_pos; j < inlet_pos + inlet_size; j++) {
                ...
                // 4.1.3. Store level in the first row of the ancillary
                // structure
                accessMat(flow, 0, j)
        ...
        if (iter % STEPS == 1) {
            ...
            for (particle = 0; particle < num_particles; particle++) {
                ...
                update_flow(
                    flow, flow_copy, particle_locations, row, col, columns, 0
                );
            ...
            for (particle = 0; particle < num_particles; particle++) {
                ...
                int back = (int)((long)particles[particle].old_flow * resistance
                                 / PRECISION)
