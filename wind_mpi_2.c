...
sect_t sector(int total, int comm_size, int rank) {
    int offset = 0, proc_items = total / comm_size, left_items = total % comm_size, displ,
        count;
    ...
    for (iter = 1; iter <= max_iter && max_var > var_threshold; iter++) {
        ...
        if (rank == 0)
            if (iter % STEPS == 1) {
                ...
                for (int particle = num_particles - num_particles_m_band;
                     particle < num_particles;
                     particle++)
                    move_particle(flow, particles, particle, rows, columns);
                /*for (int particle = particles_displs[rank]; particle < particles_displs[rank]
                 * + particles_counts[rank]; particle++) {*/
                ...
                if (rank == 0)
                    for (int particle = 0; particle < num_particles; particle++) {
                        /*for (particle = particles_displs[rank]; particle <
                         * particles_displs[rank] + particles_counts[rank]; particle++) {*/
                ...
                /*MPI_Reduce( (rank == 0 ? MPI_IN_PLACE : particle_locations),
                 * particle_locations, rows * columns, MPI_INT, MPI_SUM, 0, MPI_COMM_WORLD);*/
            ...
            if (iter % STEPS == 1) {
                ...
                for (particle = 0; particle < num_particles; particle++) {
                    ...
                    if (accessMat(particle_locations, row, col) > 0)
                        back
                            = (int)((long)particles[particle].old_flow * resistance / PRECISION)
